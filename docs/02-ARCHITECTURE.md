...
- **Sidebar panel** (`ui/shell/sidebar_panel.dart`): renders the panel for the active section (e.g. `ui/sidebar/requests_sidebar_panel.dart`).
...
- `lib/state/history_notifier.dart`: per-request response snapshots, capped at
  `AppConstants.maxHistoryEntriesPerRequest`.
- `lib/state/run_history_notifier.dart`: past Collection Runner runs.
