...
class RequestResultModel {
  ...
  RequestResultModel copyWith({int? statusCode, Map<String, List<String>>? headers, dynamic data, Duration? duration}) {
    return RequestResultModel(statusCode: statusCode ?? this.statusCode, headers: headers ?? this.headers, data: data ?? this.data, duration: duration ?? this.duration);
  }
  ...
  factory RequestResultModel.fromJson(Map<String, dynamic> json) {
    return RequestResultModel(
      ...
      headers: (json['headers'] as Map<String, dynamic>?)?.map((key, value) => MapEntry(key, List<String>.from(value as List))) ?? <String, List<String>>{},
      data: json['data'],
