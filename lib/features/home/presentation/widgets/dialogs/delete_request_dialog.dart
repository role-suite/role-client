...
class DeleteRequestDialog extends ConsumerStatefulWidget {
  const DeleteRequestDialog({super.key, required this.request});
  ...
  Future<void> _handleDelete() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Request "${widget.request.name}" deleted successfully'), backgroundColor: Colors.green));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to delete request: $e'), backgroundColor: Colors.red));
    } finally {
      if (mounted) {
