...
class _RequestChainWorkbenchTabState extends ConsumerState<RequestChainWorkbenchTab> {
  ...
  void _toggleUsePreviousResponse(int index) {
    if (index == 0) return;
    setState(() {
      _chainItems[index] = _chainItems[index].copyWith(usePreviousResponse: !_chainItems[index].usePreviousResponse);
    });
  }

  List<ApiRequestModel> _filterRequestsByCollection(List<ApiRequestModel> requests) {
    if (_selectedCollectionId == null) return requests;
    return requests.where((request) => request.collectionId == _selectedCollectionId).toList();
  }

  Future<void> _startExecution() async {
    if (_chainItems.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please add at least one request to the chain')));
      return;
    }

    final requestsAsync = ref.read(requestsNotifierProvider);
    final allRequests = requestsAsync.value ?? [];
    final chainRequests = _chainItems.map((item) => allRequests.firstWhere((r) => r.id == item.requestId)).toList();
  ...
  Widget _buildSplitLayout(BuildContext context, List<ApiRequestModel> requests, Set<String> selectedRequestIds, List<CollectionModel> collections) {
    return Row(
      ...
      children: [
            ...
            child: Column(
              ...
              children: [
                          ...
                          children: [
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('Create Request Chain', style: Theme.of(context).textTheme.titleLarge),
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Available Requests', style: Theme.of(context).textTheme.titleMedium),
                    if (collections.isNotEmpty) _buildCollectionFilter(context, collections),
                  ],
                ),
                const SizedBox(height: 8),
                Expanded(
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Card(
                              ...
                              color: isInChain ? Theme.of(context).colorScheme.primaryContainer.withValues(alpha: 0.3) : null,
                              child: ListTile(
        ...
        Container(width: 1, color: Theme.of(context).dividerColor, margin: const EdgeInsets.symmetric(vertical: 16)),
        Expanded(
          flex: 1,
          child: Container(
            padding: const EdgeInsets.all(16),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [
                Text('Chain Order', style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 8),
                Expanded(
                ...
                const SizedBox(height: 16),
                AppButton(label: 'Execute Chain', icon: Icons.play_arrow, onPressed: _chainItems.isEmpty ? null : _startExecution, isFullWidth: true),
              ],
            ),
          ),
        ),
  ...
  Widget _buildMobileLayout(BuildContext context, List<ApiRequestModel> requests, Set<String> selectedRequestIds, List<CollectionModel> collections) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Expanded(
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              leading: MethodBadge(method: request.method),
                              title: Text(request.name, maxLines: 1, overflow: TextOverflow.ellipsis),
                              subtitle: Text(request.urlTemplate, maxLines: 1, overflow: TextOverflow.ellipsis),
                              trailing: isInChain
                                  ? const Icon(Icons.check_circle, color: Colors.green)
                                  : IconButton(icon: const Icon(Icons.add_circle_outline), onPressed: () => _addRequestToChain(request), tooltip: 'Add to chain'),
  ...
  Widget _buildCollectionFilter(BuildContext context, List<CollectionModel> collections) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(Icons.filter_list, size: 18, color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.7)),
          const SizedBox(width: 8),
          DropdownButton<String?>(
            value: _selectedCollectionId,
            hint: Text('All Collections', style: Theme.of(context).textTheme.bodyMedium),
  ...
  Future<void> _loadSavedChain() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error loading saved chains: $e')));
      }
    }
  }
  ...
  Future<void> _applySavedChain(SavedRequestChain savedChain, List<ApiRequestModel> allRequests) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Loaded chain: ${savedChain.name}')));
    }
  }

  Widget _buildChainItemCard(BuildContext context, ApiRequestModel request, RequestChainItem item, int index) {
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(request.urlTemplate, style: Theme.of(context).textTheme.bodySmall, maxLines: 1, overflow: TextOverflow.ellipsis),
                    ],
                  ),
                ),
              ...
              children: [
                ...
                Expanded(
                  child: CheckboxListTile(
                    ...
                    subtitle: index == 0 ? const Text('Not available for first request') : const Text('Inject previous response body'),
                    value: item.usePreviousResponse && index > 0,
                    enabled: index > 0,
                    onChanged: index > 0 ? (value) => _toggleUsePreviousResponse(index) : null,
