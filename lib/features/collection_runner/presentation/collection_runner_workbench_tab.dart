...
class CollectionRunnerWorkbenchTab extends ConsumerStatefulWidget {
  const CollectionRunnerWorkbenchTab({super.key, this.initialCollectionId});

  final String? initialCollectionId;

  @override
  ConsumerState<CollectionRunnerWorkbenchTab> createState() => _CollectionRunnerWorkbenchTabState();
}

class _CollectionRunnerWorkbenchTabState extends ConsumerState<CollectionRunnerWorkbenchTab> {
  ...
  void _scrollToRunningRequest(List<CollectionRunResult> results) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (context != null && mounted) {
        Scrollable.ensureVisible(context, duration: const Duration(milliseconds: 300), curve: Curves.easeInOut, alignment: 0.1);
      }
    });
  }

  void _syncInitialCollectionSelection(List<CollectionModel> collections) {
    if (_didSyncInitialCollection || collections.isEmpty) return;
  ...
  Widget _buildProgressSection(BuildContext context, CollectionRunnerState state) {
    final theme = Theme.of(context);
    final label = '${state.completedRequests} of ${state.totalRequests} requests completed';
  ...
  Future<void> _exportResults(BuildContext context, CollectionRunnerState state) async {
    ...
    try {
      ...
      if (collection == null) {
        messenger.showSnackBar(const SnackBar(content: Text('No collection information available.')));
        return;
      }
