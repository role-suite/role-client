...
class CollectionRunHistoryWorkbenchTab extends ConsumerWidget {
  const CollectionRunHistoryWorkbenchTab({super.key});
    ...
    return historiesAsync.when(
      data: (histories) {
        if (histories.isEmpty) {
          return const EmptyState(icon: Icons.history, title: 'No test runs yet', message: 'Run a collection to see test history here.');
        }
    ...
    return AppCard(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(child: _StatItem(label: 'Total', value: '${history.totalRequests}', icon: Icons.list)),
              Expanded(child: _StatItem(label: 'Success', value: '${history.successfulRequests}', icon: Icons.check_circle, color: Colors.green)),
              Expanded(child: _StatItem(label: 'Failed', value: '${history.failedRequests}', icon: Icons.error, color: Colors.red)),
              Expanded(child: _StatItem(label: 'Success Rate', value: '$successRate%', icon: Icons.percent)),
            ],
          ),
          const SizedBox(height: 16),
          Row(
